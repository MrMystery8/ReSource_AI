    ...
    <div className="card card-hover overflow-hidden">
      ...
      <div className="p-6 pb-4 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-warning-50 border border-amber-500/20 flex items-center justify-center">
      ...
      <div className="px-6 pb-6 space-y-4">
        ...
        {data.identifiedHazards.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-danger-500 uppercase tracking-wide">Identified Hazards</span>
            </div>
        ...
        {data.doNotPerform.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-danger-500 uppercase tracking-wide">Do Not Perform</span>
            </div>
            <ul className="space-y-2">
              {data.doNotPerform.map((item, i) => (
                <li key={i} className="flex items-start gap-2">
                  <XCircle className="w-3.5 h-3.5 text-danger-500/70 mt-0.5 shrink-0" />
        ...
        {data.safeActions.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-success-600 uppercase tracking-wide">Safe Actions</span>
            </div>
            <ul className="space-y-2">
              {data.safeActions.map((action, i) => (
                <li key={i} className="flex items-start gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-success-500/70 mt-0.5 shrink-0" />
        ...
        {data.stopConditions.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              <StopCircle className="w-4 h-4 text-warning-500" />
              <span className="text-xs font-semibold text-warning-600 uppercase tracking-wide">Stop If...</span>
            </div>
            <ul className="space-y-2">
              {data.stopConditions.map((condition, i) => (
                <li key={i} className="flex items-start gap-2">
                  <StopCircle className="w-3.5 h-3.5 text-warning-500/70 mt-0.5 shrink-0" />

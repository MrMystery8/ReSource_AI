          ...
          {data.safeFirstActions.length > 0 && (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-semibold text-success-600 uppercase tracking-wide">Safe First Actions</span>
              </div>
              <ol className="space-y-2">
                {data.safeFirstActions.map((action, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span className="w-5 h-5 rounded-full bg-success-50 border border-emerald-500/20 flex items-center justify-center shrink-0 mt-0.5">
                      <span className="text-[10px] font-bold text-success-500">{i + 1}</span>
                    </span>
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.partsToKeep.map((part, i) => (
                    <span key={i} className="px-2 py-0.5 rounded-md bg-success-50 border border-emerald-500/20 text-[11px] text-success-600 font-medium">
                      {part}
                    </span>
                  ))}
                </div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.partsToAvoid.map((part, i) => (
                    <span key={i} className="px-2 py-0.5 rounded-md bg-danger-50 border border-rose-500/20 text-[11px] text-danger-500 font-medium">
                      {part}
                    </span>
                  ))}
                </div>
        ...
        {data.hazardWarnings.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-warning-600 uppercase tracking-wide">Hazard Warnings</span>
            </div>
            <div className="space-y-2">
              {data.hazardWarnings.map((warning, i) => (
                <div key={i} className="flex items-start gap-2">
                  <AlertTriangle className="w-3.5 h-3.5 text-warning-500/70 mt-0.5 shrink-0" />
                  <span className="text-xs text-text-secondary">
                    <span className="font-medium text-warning-600">{warning.component}</span> — {warning.risk}
                  </span>
                </div>
              ))}
            </div>
